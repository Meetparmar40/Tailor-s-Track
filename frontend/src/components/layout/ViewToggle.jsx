...
export default function ViewToggle({ setView, view }) {
  ...
  return (
    <ToggleGroup.Root
      ...
      className="inline-flex rounded-lg border border-border bg-primary-foreground dark:bg-gray-800"
    >
      {items.map(({ value, label, icon: Icon }) => (
        <ToggleGroup.Item
          key={value}
          value={value}
          className="flex gap-2 w-35 items-center justify-center px-4 py-1 text-md font-medium rounded-lg transition-colors 
